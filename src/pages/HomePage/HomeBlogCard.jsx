...
const HomeBlogCard = ({data}) => {
  ...
  const blogImage = `${appwriteStorage.getFileView(
  ...
  const fetchUserDetail=async()=>{
     try {
       ...
       setUser(documents[0])
       const profileImage = documents[0].image === 'defaultAvatar.svg' || !documents[0].image ? DEFAULT_AVATAR : `${appwriteStorage.getFileView(ENVObj.VITE_APPWRITE_STORAGE_ID,documents[0].image)}&v=${documents[0].$id}`;
...
<Link to={`/blog/${data.slug}`} className='lg:p-4 my-1'>
  <div className="bg-section border-zinc-800  border hover:border-pink-500/40  hover:scale-[101%] transition-all duration-300 w-full p-3 rounded-lg shadow-md shadow-black flex flex-col h-[500px] lg:h-full">
      <div className='h-40 lg:h-56 overflow-hidden'>
        <img className=" w-full h-full rounded-md hover:scale-105 transition-all duration-300 " src={blogImage} alt='' />
      </div>
